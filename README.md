...
Studio wristwatch timegrapher — the watch clamp, microphone/piezo slide, pivot,
shells, and enclosure parts that hold a watch against the sensor. STL and 3MF
files are print-ready; print in any rigid filament (PLA/PETG).

---

## Repository Contents

This repository contains the 3D model files, organised as follows:

- **`models/`** — print-ready `.stl` and `.3mf` parts:
  - `Watch_Clamp` — holds the watch against the pickup.
  - `Main_Slide_Body`, `Center_Column`, `Center_Pivot`, `Center_Shaft`,
    `Connector_Shaft` — the adjustable slide / pivot mechanism.
  - `Top_Shell`, `Bottom_Shell`, `End_Cover`, `Piezo_End_Mount`, `Base`,
    `LCD_Case` — enclosure and mounting parts.
...
Print settings and assembly notes can be found here:
